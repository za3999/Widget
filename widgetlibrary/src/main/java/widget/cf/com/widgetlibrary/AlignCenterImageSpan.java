...
/**
 * Created by zhengcf on 2017/7/15.
 */
...
public class AlignCenterImageSpan extends ImageSpan {
    ...
    public AlignCenterImageSpan(Context context, int resourceId) {
        super(context, resourceId);
    }

    public AlignCenterImageSpan(Context context, int resourceId, int verticalAlignment) {
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom,
        ...
        if (mVerticalAlignment == ALIGN_BASELINE) {
            transY -= fm.descent;
        } else if (mVerticalAlignment == ALIGN_CENTER) {    //此处加入判断， 如果是自定义的居中对齐
